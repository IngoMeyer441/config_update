...
function config-update () {
    ...
    function init_variables () {
        ...
        CONFIGS_WORKING_DIR="${CONFIG_UPDATE_ROOT_DIR}/configs_working_directory"
    ...
    function create_working_copy_and_fetch_remote_configs () {
    ...
    function add_config () {
        ...
        pushd "${HOME}"
        while true; do
            vared -c config_filepath
            [[ "$?" -eq 0 ]] || { popd >/dev/null 2>&1; return 7 }
            [[ -f "${config_filepath}" ]] && break
            echo -e "The file ${BOLD_LIGHT_BLUE}${config_filepath}${NC} does not exist. Please correct your input:"
        done
        config_filepath="${config_filepath:a}"
        popd >/dev/null 2>&1
        mkdir -p "${CONFIG_FILENAME:h}" || \
            { ERROR_OUTPUT="The directory ${BOLD_LIGHT_BLUE}${CONFIG_FILENAME:h}${NC} could not be created."; return 8 }
    ...
    function edit_config () {
        ...
        git checkout "${EDIT_BRANCH}" >/dev/null 2>&1 || \
            { ERROR_OUTPUT="The branch ${BOLD_LIGHT_CYAN}${EDIT_BRANCH}${NC} that should be updated cannot be checked out."; \
              return 11 }
        [[ -d "${CONFIG_FILENAME:h}" ]] || mkdir -p "${CONFIG_FILENAME:h}" || \
            { ERROR_OUTPUT="The directory ${BOLD_LIGHT_BLUE}${CONFIG_FILENAME:h}${NC} cannot be created."; return 12 }
    ...
    function merge_config () {
        function merge_config_recursive () {
    ...
    function apply_working_copy_changes () {
        local return_code filepath
